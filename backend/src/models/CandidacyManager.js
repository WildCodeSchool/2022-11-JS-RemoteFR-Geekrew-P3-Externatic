...
class CandidacyManager extends AbstractManager {
  ...
  find(candidateId) {
    return this.database.query(
      `SELECT candidacy_date, job_offer.title, job_offer.salary, job_offer.lower_salary, job_offer.work_hours, job_offer.higher_salary, job_offer.id, job_offer.description, job_offer.location, job_offer.experience, DATEDIFF(NOW(), job_offer.date_of_creation) as postDate, company.name as compname, company.field, company.picture, group_concat(technology.name SEPARATOR ', ') as technologies
      FROM candidacy 
